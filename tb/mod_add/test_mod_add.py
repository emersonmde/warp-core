"""Testbench for mod_add module.

Tests modular addition (a + b) mod 3329 with exhaustive slices,
...
async def test_exhaustive_slices(dut):
    ...
    Total: ~10k tests covering all single-variable behavior.
    """
    errors = 0
    ...
    for a in range(KYBER_Q):
        ...
        if not match:
            dut._log.error(f"FAIL: mod_add({a}, 0) = {result}, expected {expected}")
            errors += 1
    ...
    for b in range(KYBER_Q):
        ...
        if not match:
            dut._log.error(f"FAIL: mod_add(0, {b}) = {result}, expected {expected}")
            errors += 1
    ...
    for a in range(KYBER_Q):
        ...
        if not match:
            dut._log.error(f"FAIL: mod_add({a}, {a}) = {result}, expected {expected}")
            errors += 1
...
async def test_boundary_values(dut):
    ...
    cases = [
        ...
        (0, KYBER_Q - 1),              # 0 + 3328 = 3328
        (KYBER_Q - 1, 1),              # 3328 + 1 = 3329 → 0 (wraps)
        (1, KYBER_Q - 1),              # 1 + 3328 = 3329 → 0 (wraps)
        (KYBER_Q - 1, KYBER_Q - 1),    # 3328 + 3328 = 6656 → 3327
        (KYBER_Q // 2, KYBER_Q // 2),  # near-midpoint
        (KYBER_Q // 2, KYBER_Q // 2 + 1),
    ]
    errors = 0
    for a, b in cases:
        result, expected, match = await drive_and_check(dut, a, b)
        if not match:
            dut._log.error(f"FAIL: mod_add({a}, {b}) = {result}, expected {expected}")
            errors += 1
        else:
            dut._log.info(f"  mod_add({a}, {b}) = {expected}")
